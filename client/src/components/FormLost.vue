  ...
  <div class="text-center">
    <v-dialog v-model="isDialogVisible" width="700">
      <template v-slot:activator="{on, attrs}">
        <v-btn class="pa-10 red lighten-2" v-bind="attrs" v-on="on" block>
          Ztráta psa
        </v-btn>
      </template>
          ...
          <v-stepper-content step="3">
              <v-container>
                <v-card class="mb-6">
                  ...
                  <v-card-title>{{ lostDog.name }}</v-card-title>
                  <v-card-subtitle>{{ lostDog.breed }}</v-card-subtitle>
                  <v-card-text>
                    <div><b>Místo nálezu: </b> {{ lostDog.place }}</div>
                    <div><b>Pohlaví: </b>{{ lostDog.sex }}</div>
                    <div><b>Věk: </b>{{ lostDog.age }}</div>
                    <div><b>Kontaktní e-mail: </b> {{ lostDog.email }}</div>
                    <div v-if="lostDog.info"><b>Dodatečné info: </b> {{ lostDog.info }}</div>
                  </v-card-text>
                </v-card>
                <v-btn 
                  color="grey-lighten-4"
